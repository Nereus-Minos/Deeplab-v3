...
def make_var(name, shape):
    # shape = [8, 8, self.num_classes, self.num_classes]

    if len(shape) == 4:

        ins = int(shape[0] * shape[0] * shape[2])
        ins_min, ins_max = 1 / math.sqrt(ins), (-1) / math.sqrt(ins)

        return tf.get_variable(name, shape, trainable=True, initializer=tf.random_uniform_initializer(ins_min, ins_max))
...
def conv2d_transpose_strided(x, W, b, name, output_shape=None, stride=2):
    ...
    return tf.nn.bias_add(conv, b, name=name+'_add')
...
class DeepLabV3Model(object):
    ...
    def _create_network(self, image_batch):
        ...
        # ASPP层
        fc1_voc12_conv = conv(res5c_relu, 1, 256, 1, name='fc1_voc12_conv')
        fc1_voc12_conv_bn = batch_normalization(fc1_voc12_conv, is_training=self.is_training, activation_fn=None,
                                                name='fc1_voc12_conv_bn')

        fc1_voc12_c0 = atrous_conv(res5c_relu, 3, 256, 6, name='fc1_voc12_c0', biased=True)
        fc1_voc12_c0_bn = batch_normalization(fc1_voc12_c0, is_training=self.is_training, activation_fn=None,
                                              name='fc1_voc12_c0_bn')

        fc1_voc12_c1 = atrous_conv(res5c_relu, 3, 256, 12, name='fc1_voc12_c1', biased=True)
        fc1_voc12_c1_bn = batch_normalization(fc1_voc12_c1, is_training=self.is_training, activation_fn=None,
                                              name='fc1_voc12_c1_bn')

        fc1_voc12_c2 = atrous_conv(res5c_relu, 3, 256, 18, name='fc1_voc12_c2', biased=True)
        fc1_voc12_c2_bn = batch_normalization(fc1_voc12_c2, is_training=self.is_training, activation_fn=None,
                                              name='fc1_voc12_c2_bn')

        # 图像级特征（即全局平均池化）
        # print(res5c_relu.shape)
        image_pool = tf.reduce_mean(res5c_relu, [1, 2], name='fc1_image_pool')
        # print(image_pool.shape)
        image_pool = tf.expand_dims(image_pool, axis=1)
        image_pool = tf.expand_dims(image_pool, axis=1)
        # print(image_pool.shape)
        image_pool = conv(image_pool, 1, 256, 1, name='fc1_image_pool_conv')
        image_pool_bn = batch_normalization(image_pool, is_training=self.is_training, activation_fn=None,
                                                name='fc1_image_pool_bn')
        image_pool = tf.image.resize_bilinear(image_pool_bn, tf.shape(res5c_relu)[1:3])

        fc1_voc12 = tf.concat([fc1_voc12_conv_bn, fc1_voc12_c0_bn, fc1_voc12_c1_bn, fc1_voc12_c2_bn, image_pool], 3, name='fc1_voc12')

        raw_net = conv(fc1_voc12, 1, 256, 1, name='fc1_raw_net')
        raw_net_bn = batch_normalization(raw_net, is_training=self.is_training, activation_fn=None,
                                                name='fc1_raw_net_bn')

        net = conv(raw_net_bn, 1, self.num_classes, 1, name='fc1_net')

        return net

    def loss(self, image_batch, label_batch, weight_decay):
        raw_output = self._create_network(tf.cast(image_batch, tf.float32))

        # *****做八倍上采样，在V3中使用的原始图像大小与label做loss
        raw_output = tf.image.resize_bilinear(raw_output, label_batch.get_shape().as_list()[1:3])
        ...
        return raw_output, reduced_loss
